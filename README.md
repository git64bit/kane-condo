...
Batch 034 added the local browser runtime and package validation. Batch 035 added the fitted full-county opening outline. Batch 036 added continuous browser-side pan and pointer-anchored zoom. Batch 037 added progressive browser-side road rendering from `roads-lod.krf`. Batch 038 added progressive browser-side water rendering from `water-lod.krf`. Batch 039 added progressive building rendering from `buildings-lod.krf`. Batch 040 applies the validated offline `classification-snapshot.json` to those project-owned building identities so Unclassified, Other, Condominium, and Apartments render gray, red, green, and yellow respectively. Batch 041 adds independent browser-memory visibility controls for those four classes; all begin visible on every launch, and hiding a class changes only transient rendering.

The application logic remains browser-side HTML, CSS, and JavaScript. The current Python loopback runtime is a local static-serving mechanism, not an application dependency or authoritative backend. Road, water, and building KRF parsing, zlib decompression, validation, LOD selection, and SVG rendering occur in the browser; the host only serves static files. Batch 040 joins the sparse classification snapshot to decoded buildings by project-owned `building_key`; absent classification records safely remain Unclassified gray. Batch 041 keeps class visibility as transient browser-memory state shared through one visibility predicate for later hit testing. The join, color grouping, and filtering remain browser-side and read-only; no package or server state is changed.

## Development boundary

Batches 008–015 establish the GeoPackage core and authoritative data model. Batches 016–024 establish refresh detection, candidate processing, reconciliation, promotion, and rollback. Batches 025–033 establish the reproducible offline render package. Batches 034–041 establish the initial offline, read-only browser application, full-county opening view, continuous navigation, and progressive road/water/building rendering while preserving the separation between local rendering and the future private authoritative server.
